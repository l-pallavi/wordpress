...
/**
 * Class SearchWPModalFormSettings.
 *
 * The SearchWP Modal Search Form settings.
 *
 * @since 0.5.0
 */
class SearchWPModalFormSettings {
	...
	public function assets() {
		...
		if ( ! Utils::is_searchwp_active() ) {
			...
			wp_enqueue_style(
				...
				SEARCHWP_MODAL_FORM_PLUGIN_URL . 'assets/vendor/fontawesome/css/font-awesome.min.css',
				null,
				'4.7.0'
			);
		}
	...
	private function get_settings_fields() {
		...
		return apply_filters( 'searchwp_modal_form_settings_fields', $fields );
	}
		...
		<div class="swp-content-container">
			...
			<p class="submit">
				<button type="submit" form="searchwp-modal-form-admin-settings-form" class="searchwp-btn searchwp-btn-md searchwp-btn-accent" name="searchwp-modal-form-settings-submit">
					<?php esc_html_e( 'Save Settings', 'searchwp-modal-search-form' ); ?>
				</button>
			</p>
	...
	private function output_form() {
		...
		<form id="searchwp-modal-form-admin-settings-form" class="searchwp-admin-settings-form" method="post">
			...
			<input type="hidden" name="nonce" value="<?php echo esc_attr( wp_create_nonce( 'searchwp-modal-form-settings-nonce' ) ); ?>">
			<?php
	...
	private function output_field_checkbox( $args ) {
		...
		$value   = searchwp_modal_form()->get( 'SettingsApi' )->get( $args['slug'], $default );
	...
	private function output_field_select( $args ) {
		...
		$default = isset( $args['default'] ) ? esc_html( $args['default'] ) : '';
		$slug    = sanitize_key( $args['slug'] );
		$value   = searchwp_modal_form()->get( 'SettingsApi' )->get( $slug, $default );
	...
	public function header_searchwp_disabled() {
		...
		do_action( 'searchwp_modal_form_settings_header_before' );
		...
		do_action( 'searchwp_modal_form_settings_header_after' );
	}
	...
	private static function header_searchwp_disabled_main() {
        ...
        <div class="searchwp-settings-header">
            ...
            <div class="searchwp-header-actions">
				<?php do_action( 'searchwp_modal_form_settings_header_actions' ); ?>
            </div>
        </div>
		<?php
	}
                ...
                <ul>
                    <li class="searchwp-settings-nav-tab-wrapper searchwp-settings-nav-tab-active postbox-wrapper searchwp-settings-nav-tab-searchwp-live-search-wrapper">
                        <a href="https://searchwp-plugin.local/wp-admin/admin.php?page=searchwp-live-search" class="searchwp-settings-nav-tab searchwp-settings-nav-tab-active postbox searchwp-settings-nav-tab-searchwp-live-search">
                            <span><?php esc_html_e( 'Settings', 'searchwp-modal-search-form' ); ?></span>
                        </a>
                    </li>
                </ul>
            </nav>
	...
	public function page_searchwp_disabled() {
		...
		searchwp_modal_form()->get( 'Settings' )->output();
	}
    ...
    private function output_after_settings() {
        ...
        <div class="searchwp-settings-cta">
            <h5><?php esc_html_e( 'Get SearchWP Pro and Unlock all the Powerful Features', 'searchwp-modal-search-form' ); ?></h5>
            <p><?php esc_html_e( 'Thank you for being a loyal SearchWP Modal Search Form user. Upgrade to SearchWP Pro to unlock all the powerful features and experience why SearchWP is the best WordPress search plugin.', 'searchwp-modal-search-form' ); ?></p>
            <p>
			    <?php
			    printf(
				    wp_kses( /* translators: %s - star icons. */
					    esc_html__( 'We know that you will truly love SearchWP Pro. It’s used on over 30,000 smart WordPress websites and is consistently rated 5-stars (%s) by our customers.', 'searchwp-modal-search-form' ),
					    [
						    'i' => [
							    'class'       => [],
							    'aria-hidden' => [],
            ...
            <h6><?php esc_html_e( 'Pro Features:', 'searchwp-modal-search-form' ); ?></h6>
            <div class="list">
                <ul>
                    <li><?php esc_html_e( 'Search all custom field data', 'searchwp-modal-search-form' ); ?></li>
                    <li><?php esc_html_e( 'Make ecommerce metadata discoverable in search results', 'searchwp-modal-search-form' ); ?></li>
                    <li><?php esc_html_e( 'Search PDF, .doc, .txt and other static documents', 'searchwp-modal-search-form' ); ?></li>
                    <li><?php esc_html_e( 'Search custom database tables and other custom content', 'searchwp-modal-search-form' ); ?></li>
                    <li><?php esc_html_e( 'Make your media library (images, videos, etc.) searchable', 'searchwp-modal-search-form' ); ?></li>
                </ul>
                <ul>
                    <li><?php esc_html_e( 'Search categories, tags and even custom taxonomies', 'searchwp-modal-search-form' ); ?></li>
                    <li><?php esc_html_e( 'Easy integration with all WordPress themes and page builders', 'searchwp-modal-search-form' ); ?></li>
                    <li><?php esc_html_e( 'Advanced search metrics and insights on visitor activity', 'searchwp-modal-search-form' ); ?></li>
                    <li><?php esc_html_e( 'Multiple custom search engines for different types of content', 'searchwp-modal-search-form' ); ?></li>
                    <li><?php esc_html_e( 'WooCommerce & Easy Digital Downloads support', 'searchwp-modal-search-form' ); ?></li>
                </ul>
            </div>
            <p><a href="https://searchwp.com/?utm_source=WordPress&utm_medium=Settings+Upgrade+Bottom+Link&utm_campaign=Modal+Search+Form&utm_content=Get+SearchWP+Pro+Today+and+Unlock+all+the+Powerful+Features" target="_blank" rel="noopener noreferrer" title="<?php esc_html_e( 'Get SearchWP Pro Today', 'searchwp-modal-search-form' ); ?>"><?php esc_html_e( 'Get SearchWP Pro Today and Unlock all the Powerful Features', 'searchwp-modal-search-form' ); ?> &raquo;</a></p>
            <p>
	            <?php
	            echo wp_kses(
		            __( '<strong>Bonus:</strong> SearchWP Modal Search Form users get <span class="green">50% off the regular price</span>, automatically applied at checkout!', 'searchwp-modal-search-form' ),
		            [
			            'strong' => [],
			            'span'   => [
				            'class' => [],
	...
	public function admin_footer_rate_us_searchwp_disabled( $text ) {
		...
		$url = 'https://wordpress.org/support/plugin/searchwp-modal-search-form/reviews/?filter=5#new-post';

		return sprintf(
			wp_kses( /* translators: $1$s - SearchWP plugin name; $2$s - WP.org review link; $3$s - WP.org review link. */
				__( 'Please rate %1$s <a href="%2$s" target="_blank" rel="noopener noreferrer">&#9733;&#9733;&#9733;&#9733;&#9733;</a> on <a href="%3$s" target="_blank" rel="noopener">WordPress.org</a> to help us spread the word. Thank you from the SearchWP team!', 'searchwp-modal-search-form' ),
				[
					'a' => [
						'href'   => [],
						'target' => [],
			...
			'<strong>SearchWP Modal Search Form</strong>',
			$url,
			$url
		);
	}
	...
	public static function get_dyk_block_output() {
        ...
        <div class="searchwp-settings-dyk">
            <h5><?php esc_html_e( 'Did You Know?', 'searchwp-modal-search-form' ); ?></h5>
            <p>
	            <?php
	            echo wp_kses(
		            __( 'By default, most of your site content is hidden from search results. <strong><em>That’s frustrating</em></strong>, because it leaves your visitors unable to find what they are searching for - meaning lost subscribers, leads and sales for you!', 'searchwp-modal-search-form' ),
		            [
			            'strong' => [],
			            'em'     => [],
		            ]
            ...
            <p><?php esc_html_e( 'With SearchWP Pro, you can grow your business faster by making all of your site content searchable, such as custom fields, ecommerce data, categories, PDF documents, rich media and more!', 'searchwp-modal-search-form' ); ?></p>
            <p><a href="https://searchwp.com/?utm_source=WordPress&utm_medium=Settings+Did+You+Know+Upgrade+Link&utm_campaign=Modal+Search+Form&utm_content=Get+SearchWP+Pro+Today" target="_blank" rel="noopener noreferrer" title="<?php esc_html_e( 'Get SearchWP Pro Today', 'searchwp-modal-search-form' ); ?>"><?php esc_html_e( 'Get SearchWP Pro Today', 'searchwp-modal-search-form' ); ?> &raquo;</a></p>
            <p>
	            <?php
	            echo wp_kses(
		            __( '<strong>Bonus:</strong> SearchWP Modal Search Form users get <span class="green">50% off the regular price</span>, automatically applied at checkout!', 'searchwp-modal-search-form' ),
