...
class InstagramTemplateHandler
{
    ...
    public function renderPostMedia($feed = [], $template_meta = [], $index = null)
    {
        ...
        $app->view->render('public.feeds-templates.instagram.elements.media', [
            ...
            'placeholder_img_class' => ($media_url && !str_contains($media_url, 'placeholder') ? 'wpsr-show' : 'wpsr-hide'),
            'animation_img_class' => ($media_url && (str_contains($media_url, 'placeholder')) || $layout_type === 'carousel') ? 'wpsr-animated-background' : '',
            'index'         => $index
        ]);
    }
